...
const Inbox = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://mail-box-client-2b50e-default-rtdb.firebaseio.com/inbox/${sentMail}.json`
        );
  ...
  const readHandler = async (id) => {
    ...
    try {
      await axios.patch(
        `https://mail-box-client-2b50e-default-rtdb.firebaseio.com/inbox/${sentMail}/${id}.json`,
        {
          classes: true,
        }
      );
